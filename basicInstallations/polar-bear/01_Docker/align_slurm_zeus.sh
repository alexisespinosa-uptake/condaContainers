...
#AEG: Singularity image
theImage="/group/director2150/singularity/groupRepository/polar/polar-0_213-pawsey.sif"

#AEG: In the following lines the "commands" are going to be modified to make use of the container within zeus
#     (Original lines are commented with the "#@AEG@" marker)
#     (New lines are guided by the singularity usage at Pawsey, and
#      by the commands used in the .../Polar/align_serial.sh script)

## Software commands; add load modules as necessary
# BWA for alignment to different genomes
#@AEG@LOAD_BWA="spack load bwa@0.7.17 arch=\`spack arch\`"
#@AEG@BWA_CMD="bwa"
LOAD_BWA="module load singularity"
BWA_CMD="singularity exec $theImage bwa"
# Samtools for file manipulation
#@AEG@LOAD_SAMTOOLS=""
#@AEG@SAMTOOLS_CMD="samtools"
LOAD_SAMTOOLS="module load singularity"
SAMTOOLS_CMD="singularity exec $theImage samtools"
# Megahit for contig creation
#@AEG@LOAD_MEGAHIT=""
#@AEG@MEGAHIT_CMD="/gpfs0/work/brian/scripts/MEGAHIT-1.2.9-Linux-x86_64-static/bin/megahit"
LOAD_MEGAHIT="module load singularity"
MEGAHIT_CMD="singularity exec $theImage megahit"
# Minimap2 for creating dotplot
#@AEG@LOAD_MINIMAP2=""
#@AEG@MINIMAP2_CMD="minimap2"
LOAD_MINIMAP2="module load singularity"
MINIMAP2_CMD="singularity exec $theImage minimap2"
# Python for creating dotplot
#@AEG@LOAD_PYTHON=""
#@AEG@PYTHON_CMD="/gpfs0/apps/x86/anaconda3/bin/python"
LOAD_PYTHON="module load singularity"
PYTHON_CMD="singularity exec $theImage python"
...
#AEG: Times for the slurm jobs
#     None of these variables were originally present in the original script.
#     Originally, times were hardcoded in each section.
#     If different times for each section are needed, we recommend to
#     create more variables and use them in the each of the slurm sections
maxJobTime=10
medJobTime=5
shortJobTime=3

#AEG: Memory for the slurm jobs
#     Memory settings for each of the slurm sections are still hardcoded (as in the original script)
#     If different memory settings for each section are needed, we recommend to
#     create more variables and use them in the each of the slurm sections

#AEG: Names slurm jobs
#     I added name strings for the slurm jobs (-J option) in all the sections
#     that did not have this setting originally
...
#@AEG@echo "#SBATCH -t 30 >> "$WORK_DIR"/collect_stats.sh
echo "#SBATCH -t $shortJobTime" >> "$WORK_DIR"/collect_stats.sh
...
echo "#SBATCH -J \"collectstats\"" >> "$WORK_DIR"/collect_stats.sh #AEG
